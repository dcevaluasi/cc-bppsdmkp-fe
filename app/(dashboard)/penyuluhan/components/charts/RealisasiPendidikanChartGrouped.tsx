...
const RealisasiPendidikanChartGrouped: React.FC<RealisasiPendidikanChartGroupedProp> = ({ loading, title, jenjang, data, config, isFull }) => {
    return loading ? (
...
type Props = {
    title: string;
    jenjang: { name: string; value: number; fill: string }[];
...
function CardBarChart({
    ...
    const chartKeys = ['pesertaDidik', 'alumni', 'pendidik', 'tenagaPendidik',] as const;

    type ChartKey = typeof chartKeys[number];

    const chartConfig: Record<ChartKey, { label: string; color: string }> = {
        pesertaDidik: { label: "Peserta Didik", color: "#EF4444" },
